...
class SSOService:
    ...
    def authenticate(self, encrypted_payload: str) -> Dict[str, Any]:
        ...
        logger.info("Received GHL SSO payload")

        try:
            user_data = decrypt_sso_payload(
                encrypted_payload,
                self.shared_secret,
            )
        except Exception as exc:
            logger.exception("Failed to decrypt GHL SSO payload")
            raise ValueError(
                f"Failed to decrypt GHL SSO payload: {exc}"
            ) from exc

        if not isinstance(user_data, dict):
            raise ValueError("Decrypted SSO payload must be a JSON object")

        logger.info(
            "Successfully decrypted GHL SSO payload for user %s",
        ...
        logger.info(
            "Created SSO session for user %s",
            user_data.get("userId"),
        )
    ...
    def _validate_user_data(
        ...
        missing = [
            ...
            if user_data.get(field) in (None, "")
        ]

        if missing:
            raise ValueError(
                "Decrypted SSO payload is missing required fields: "
                + ", ".join(missing)
            )